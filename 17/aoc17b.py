...
# it would be ideal if we could flip going down and right each turn.
    ...
    for i in range(len(new_facing)):
        ...
        new_max = FindPath(new_facing[i][1],new_facing[i][0],0,map,my,mx,heat,depth+1,max)
        if(new_max < max):
            max = new_max
...
#    print(heatmap)
    ...
    facing = ">"
    new_max = FindPath((0,0),facing,0,map,my,mx,0,0,max)
    facing = "v"
    new_max = FindPath((0,0),facing,0,map,my,mx,0,0,new_max)
